...
case "${MODE}" in
  smoke|regression) ;;
  *) printf "Usage: %s [smoke|regression]\n" "$0" >&2; exit 2 ;;
esac
...
print(tokens if isinstance(tokens, int) and tokens > 0 else 0)
PY
  )"
  if [ "${index}" = "warmup" ]; then
    return 0
  fi
  python3 - "${first}" "${total}" "${completion_tokens}" >>"${TMP_DIR}/samples.txt" <<'PY'
import math
import sys

first, total = map(float, sys.argv[1:3])
...
print(f"{first * 1000:.3f} {total * 1000:.3f} {tokens}")
PY
}
...
read -r TTFB_P50_MS TTFB_P95_MS TOTAL_P50_MS TOTAL_P95_MS TOKENS_PER_SECOND < <(
...
token_rates = [row[2] / (row[1] / 1000) for row in rows if row[2] > 0 and row[1] > 0]
token_rate = f"{percentile(token_rates, .50):.3f}" if len(token_rates) == len(rows) else "NOT_MEASURED"
print(
    f"{percentile(first, .50):.3f} {percentile(first, .95):.3f} "
    f"{percentile(total, .50):.3f} {percentile(total, .95):.3f} {token_rate}"
)
PY
)
...
if [ "${MODE}" = "regression" ]; then
  if [ "${TOKENS_PER_SECOND}" = "NOT_MEASURED" ] || [ "${MEMORY_MB}" = "NOT_MEASURED" ]; then
    printf "Regression benchmark requires measured completion tokens and container memory.\n" >&2
    exit 1
  fi
fi

HOST_KERNEL="$(uname -sr 2>/dev/null || printf 'NOT_MEASURED')"
HOST_ARCH="$(uname -m 2>/dev/null || printf 'NOT_MEASURED')"
HOST_CPU_COUNT="$(getconf _NPROCESSORS_ONLN 2>/dev/null || printf 'NOT_MEASURED')"
GENERATED_TOKENS="$(
  awk '{ total += $3 } END { print total + 0 }' "${TMP_DIR}/samples.txt"
)"
...
- Model SHA-256: NOT_VERIFIED
- Runtime implementation: llama.cpp server
- Runtime version: NOT_VERIFIED
- Runtime revision: NOT_VERIFIED
- Runtime flag schema: llama-server-cli-v1
- Runtime compatibility: static-template-only
- Serving image: ghcr.io/ggml-org/llama.cpp:server@sha256:{GENERATION_FINGERPRINT}
...
- Workload: POST /v1/chat/completions; prompt="Respond with OK."; max_tokens=8
- Effective config: ctx_size=4096, batch_size=256, threads=6, n_predict=256
- Concurrency: 1
- Generated completion tokens: ${GENERATED_TOKENS}
...
| Tokens per second p50 | ${TOKENS_PER_SECOND} |

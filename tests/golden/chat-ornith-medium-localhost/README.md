...
- Runtime image: `ghcr.io/ggml-org/llama.cpp:server@sha256:{GENERATION_FINGERPRINT}`
...
Compose mounts the host model path read-only at the container model path.
Do not copy the model into this workspace. Before startup, run the host
validation command below to verify that the original host file and

...
| APR-036 | 2026-07-25 | SEC7-001 | `harden gate` | Gate 1 and supply-chain rules do not prohibit CI from hashing or scanning a historical audit directory outside the current run. | M | CI receives a versioned current-run evidence manifest and fails when a declared audit input resolves outside that run or its canonical source set. | open |
| APR-037 | 2026-07-25 | OPS7-001 | `harden gate` | The same historical-audit dependency is independently reported as an operational reproducibility defect and needs its own finding-level trace. | M | The CI configuration test rejects historical audit paths and verifies that checksum and scanner inputs belong to the current run or versioned canonical sources. | open |
| APR-038 | 2026-07-25 | HARNESS7-001 | `harden gate` | `agents check` only tests file existence; existing role and conformance rules do not make content/materialization integrity mechanically fail closed. | M | Agent checks validate marker, frontmatter, native format, role, tools, platform, skill and atomic materialization, with malformed or stale fixtures failing. | open |
